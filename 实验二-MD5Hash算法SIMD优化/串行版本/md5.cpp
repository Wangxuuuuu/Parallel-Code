...
void MD5Hash(string input, bit32 *state)
{
	...
	for (int i = 0; i < n_blocks; i += 1)
	{
		...
		for (int j = 0; j < 16; ++j)
		{
			x[j] =  //i*64：表示第 i 块在整个填充后消息里的起始字节位置。每块正好 64 字节。
					//4*j + k（k 从 0 到 3）：在第 i 块内部，又把 64 字节分成 16 份，每份 4 字节（4×16 = 64）。j 表示这是第几个 4 字节组，k 表示在 这 4 字节组里取第几个字节。
					// 取第 i 块的第 j 个字节组的第 0 个字节（最低位）
					(paddedMessage[4 * j + i * 64 + 0]) |
					// 取第 1 个字节，左移 8 位，组成 bits 8–15
					(paddedMessage[4 * j + i * 64 + 1] << 8) |
					// 取第 2 个字节，左移 16 位，组成 bits 16–23
					(paddedMessage[4 * j + i * 64 + 2] << 16) |
					// 取第 3 个字节，左移 24 位，组成 bits 24–31
					(paddedMessage[4 * j + i * 64 + 3] << 24);
		}
