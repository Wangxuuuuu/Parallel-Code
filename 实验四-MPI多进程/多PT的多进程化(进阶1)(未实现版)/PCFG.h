...
// 用于存储单个PT处理结果的结构体
struct PTResult {
    vector<string> guesses;        // 生成的猜测
    vector<PT> new_pts;           // 新生成的PT
    int process_id;               // 处理该PT的进程ID
};
...
public:
    ...
    // 将优先队列最前面的一个PT出队（原有方法）
    void PopNext();
    
    // **新增：多进程并行处理多个PT**
    void PopMultiple(int num_pts = 4);
    
    // **新增：单个进程处理一个PT的函数**
    PTResult ProcessSinglePT(PT pt, int process_id);
    
    // **新增：将多个新PT插入优先队列**
    void InsertMultiplePTs(const vector<PT>& new_pts);
...
private:
    // **新增：序列化PT对象用于进程间通信**
    string SerializePT(const PT& pt);
    PT DeserializePT(const string& data);
    
    // **新增：通过管道发送和接收数据**
    void SendPTResult(int pipe_fd, const PTResult& result);
    PTResult ReceivePTResult(int pipe_fd);
};
